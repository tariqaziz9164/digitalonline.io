...
#define MOSFET_OFF digitalWrite(IN_pin,LOW)   // here an optocupler PC817 or any other,just turn off its input.
...
float error1 = 0;                 // error during maintain absorption voltage
float error2 = 0;                //error during maintain float voltage
...
unsigned long boost_settime= 60;  // by default
...
void run_MPPTcharger()  { 
                                                     ...
                                                     if(((Vbatt>12)&&(Vsolar< Vbatt+0.5)||(Vbatt<= 12)&&(Vsolar<12.5)||(Isolar<0.1)||(Ibatt>Imax)||(Vbatt>Vboost+0.15)||Psolar<Pleast))
                                                             // || Tbatt>Tmax )) if temperatur sensor is use then uncomment and include in if statment
                                                           {
                                                            ...
                                                            break; 
                                                                ...
                                                                {
                                                                 if(boost_time>boost_settime) { //boost set time 60
                                                                                               charger_mode = float_mode;
                                                                                              }
                                                                   else 
                                                                       {
                                                                         if (Vbatt>Vboost) { // vboost is 14.5
                                                                     ...
                                                                     if(Vbatt>Vboost+ 0.02)
                                                                                    {
                                                                                      ...
                                                                                      inc = delta*500*error1;  //if battery voltage > 14.5 the pwm to be decrease by % error.the pwm is zero 
                                                                                                                 //at 14.6
                                                                                      pwm-=inc;
                                                                                      set_pwmdutycycle(pwm);
                                                                                      boost_time =  boost_time+millis()-boost_millis;                                                    
  ...
  void mppt_INCCOND()
                     ...
                     void load_engage()
                                       {
                                          if(Vbatt>LVD)                                                 if(Psolar<Pleast && Vbatt>LVD)
                                                          {LOAD_ON;}

                                                                    //  else
                                             else if(Vbatt<=LVD)                          //   {
                                                          { LOAD_OFF; }             //    if(Psolar>Pleast && Vbatt>Vfloat) { LOAD_ON;}
                                                                        //    else {LOAD_OFF;}
                                                                        //  }
                                                                        // LOAD_OFF;
                                       }
